...
def parse_args():
    ...
    general.add_argument('--cuda', action='store_true',
                         help='Use CUDA')
    general.add_argument('--fp16', type=str, default="O1",
    ...
    model.add_argument('--attn_type', type=int, default=0,
                       ...
                       '2 for Vaswani et al, 3 for Al Rfou et al, 4 for no position embedding, 5 for Linear attention,\
                       6 for luna attention. 7 for lahm with disentangled bias. 71 for lahm with xl rpe. 72 for lahm with disentangled scaling. \
                       73 for lahm with disentangled scaling + key interaction.')
    model.add_argument('--attn_norm_type', type=int, default=0,
                       help='Attention norm type. 0 for traditional accumulated normalization. 1 for vector normalization')
    model.add_argument('--half_loss', action='store_true',
                       help='Only compute loss for recent half of the segment. Tricks to train absolute position embedding')
    model.add_argument('--luna_tie_kv', action='store_true',
                       help='Luna attn: tie kv proj to save parameters')
    model.add_argument('--luna_parallel', action='store_true',
                       help='Luna attn: invoke parallel causal attention using cumsum')
    model.add_argument('--luna_proj_len', type=int, default=16,
                       help='Luna attn: length of p')
    model.add_argument('--enhance_recurrence', action='store_true',
                       help='Use enhance recurrence from ERINE-doc, i.e., mems = [s^n_{t-1}, s^{n-1}_t]')
    model.add_argument('--calc_mem_loss', action='store_true',
                        help='Calculate memory loss for LAHM')
    ...
    for batch, (data, target, seq_len) in enumerate(train_iter):
        ...
        if args.batch_chunk > 1:
            ...
            for i in range(args.batch_chunk):
                ...
                ret = para_model(data_i, target_i, *mems[i], mem_target=prev_target)
                loss, mem_loss, mems[i] = ret[0], ret[1], ret[2:]
            ...
            ret = para_model(data, target, *mems, mem_target=prev_target)
            loss, mem_loss, mems = ret[0], ret[1], ret[2:]
            loss = loss.float().mean().type_as(loss)
            mem_loss = mem_loss.float().mean().type_as(mem_loss)

            total_loss = (loss + mem_loss) / 2
            if args.fp16:
            ...
            train_loss += loss.float().item()
            train_mem_loss += mem_loss.float().item()
        
        if args.calc_mem_loss:
        ...
        if train_step % args.log_interval == 0:
            ...
            cur_mem_loss = train_mem_loss / log_step
            cur_mem_loss = utils.distributed.all_reduce_item(cur_mem_loss, op='mean')
            train_mem_loss = 0
            ...
            log_str = '| epoch {:3d} step {:>8d} | batches {:>6d} / {:d} | lr {:.3e} ' \
                '| ms/batch {:5.1f} | tok/s {:>7d} | mem_loss {:5.2f} | loss {:5.2f}'.format(
                    epoch,
                    train_step,
                    batch+1,
                    tr_iter.n_batch,
...
def main():
    ...
    args.work_dir = utils.exp_utils_ddp.build_work_dir_name(args.work_dir,
                                                        args.dataset,
                                                        args.append_dataset,
                                                        args.append_time,
                                                        )
    ...
    with utils.distributed.sync_workers() as rank:
        if rank == 0:
            create_exp_dir(args.work_dir,
                           scripts_to_save=['train_ddp.py', 'mem_transformer.py'],
                           debug=args.debug)
    ...
    model_config = {
        ...
        'half_loss': args.half_loss,
        'enhance_recurrence': args.enhance_recurrence
        }
    ...
    if args.fp16:
        model, optimizer = amp.initialize(
            model,
            optimizer,
            opt_level=args.fp16, # O1: mixed precision, O2: almost fp16
            )
